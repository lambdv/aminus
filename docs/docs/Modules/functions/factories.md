...
Factory for creating stattables from file system data.

## Description

This module provides the `StatFactory` struct with methods to read character and weapon base stats from JSON files, as well as artifact main stat and substat values based on rarity and level.

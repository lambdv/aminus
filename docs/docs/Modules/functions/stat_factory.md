...
Factory for creating stattables with cached data and async API support.

## Description

This module provides the `StatFactory` struct with methods to get character and weapon base stats from cached JSON data, as well as async methods to fetch data from the Irminsul API. It also provides artifact main stat and substat values based on rarity and level.
